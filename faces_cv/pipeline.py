...
The user-facing layer of the SDK: :func:`track_video` for the common case and
:class:`VideoTracker` for custom detector/tracker injection. Both return a
:class:`TrackingResult` that yields per-frame tracks, a per-track metrics table,
file exports, and an annotated render.
"""
...
from .detection import Detector, UltralyticsDetector
from .tracking import ByteTrackTracker, CATEGORY_BY_CLASS, Track, Tracker, TARGET_CLASSES
...
class VideoTracker:
    ...
    Both components are pluggable: any :class:`~faces_cv.detection.Detector` and any
    :class:`~faces_cv.tracking.Tracker` work. Frames are fed in order; the tracker is
    reset at the start of each ``run`` so results are independent across calls.
    """
...
def _classes_for(targets: tuple[str, ...]) -> list[int]:
...
def track_video(
    ...
    *,
    ...
    :class:`~faces_cv.detection.UltralyticsDetector` and a
    :class:`~faces_cv.tracking.ByteTrackTracker` (unless ``tracker`` is supplied) and runs
    them through :class:`VideoTracker`.

    ``peek_first`` runs a fast :func:`peek_video` pre-scan and, if the clip isn't
    interesting, short-circuits — returning an empty :class:`TrackingResult` (no frames)
    without the full decode + track. Useful for skipping dead clips in batch runs.
    """
    if peek_first:
    ...
    detector = UltralyticsDetector(
        weights=weights, classes=_classes_for(targets), conf=conf, device=device
    )
...
@dataclass
class PeekResult:
    """Verdict from a fast :func:`peek_video` pre-scan of a clip."""
    ...
    min_hits: int
    elapsed_s: float = 0.0  # decode + detect time for this clip (excludes detector build)
...
def _sample_frames(video_path, *, n_samples: int, sample_width: int | None = None):
    ...
    Decodes sequentially and only fully retrieves (colour-converts) every ``stride``-th
    frame — ``grab()`` skips the rest cheaply. Random-access seeking is *slower* for H.264
    (it decodes forward from the nearest keyframe), and keyframes are too few/uneven to
    sample on. When ``sample_width`` is set, each kept frame is downscaled to that longest
    side (peek only needs category presence, not box coordinates), which bounds memory.
...
def peek_video(
    ...
    *,
    ...
    fast model (``yolo11n``) at reduced inference resolution (``imgsz=320``); frames are
    reached by seeking, so cost scales with ``n_samples`` not clip length. Pass a custom
    ...
    Sampling decodes the clip sequentially and only fully retrieves every ``stride``-th
    frame (``stride`` derived to spread ~``n_samples`` evenly across the clip). This beats
    random-access seeking: H.264 ``CAP_PROP_POS_FRAMES`` decodes forward from the nearest
    keyframe, so sparse seeks cost *more* than one linear pass — and keyframes themselves
    are too few/uneven to sample on. ``grab()`` skips the colour-convert for skipped frames.
    """
    video_path = Path(video_path)
    if detector is None:
        detector = UltralyticsDetector(
            weights=weights, classes=_classes_for(targets), conf=conf,
...
def peek_videos(
    ...
    *,
    ...
    (``max_workers``) — decode is the bottleneck and releases the GIL, so this scales; no
    detector call happens here. **Phase 2** runs inference on *all* sampled frames in one
    batched pass (``batch_size`` per ``predict``) on the calling thread — collapsing the
    hundreds of Python-heavy per-frame calls that bottlenecked the per-clip approach.
    ...
    detector = detector or UltralyticsDetector(
        weights=weights, classes=_classes_for(targets), conf=conf, device=device, imgsz=imgsz
    )
