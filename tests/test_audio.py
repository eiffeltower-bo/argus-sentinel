...
def test_analyze_audio_to_dict_is_jsonable():
    ...
    res = analyze_audio(
        Path("clip.wav"),
        classifier=fake,
        _samples=(np.zeros(12 * SR, np.float32), SR),
        segment_seconds=5.0,
        overlap_seconds=1.0,
    )
    d = res.to_dict()
...
# ---- default model = zero-shot CLAP + its default label vocabulary --------------------


class _RecordingZeroShot:
    """Zero-shot fake that records the ``candidate_labels`` each segment was classified with."""

    classifier_id = "fake_clap"
    is_zero_shot = True

    def __init__(self):
        self.seen_labels: list = []

    def classify(self, samples, samplerate, *, top_k: int = 2, candidate_labels=None):
        self.seen_labels.append(candidate_labels)
        return [AudioPrediction((candidate_labels or ["?"])[0], 0.5)]


def test_default_model_is_zero_shot_clap():
    from argus import DEFAULT_AUDIO_MODEL, DEFAULT_CANDIDATE_LABELS

    assert "clap" in DEFAULT_AUDIO_MODEL.lower()
    assert "gunshot" in DEFAULT_CANDIDATE_LABELS  # surveillance-oriented set


def test_zero_shot_uses_default_labels_when_none_given():
    from argus.pipeline.audio import DEFAULT_CANDIDATE_LABELS

    fake = _RecordingZeroShot()
    analyze_audio(
        Path("clip.wav"),
        classifier=fake,
        _samples=(np.zeros(6 * SR, np.float32), SR),
        segment_seconds=5.0,
        overlap_seconds=1.0,
    )
    assert fake.seen_labels and all(lbls == DEFAULT_CANDIDATE_LABELS for lbls in fake.seen_labels)


def test_zero_shot_explicit_labels_override_default():
    fake = _RecordingZeroShot()
    analyze_audio(
        Path("clip.wav"),
        classifier=fake,
        _samples=(np.zeros(6 * SR, np.float32), SR),
        candidate_labels=["only this"],
        segment_seconds=5.0,
        overlap_seconds=1.0,
    )
    assert all(lbls == ["only this"] for lbls in fake.seen_labels)


def test_non_zero_shot_classifier_gets_no_default_labels():
    # The fixed-label fake (is_zero_shot=False) must NOT be handed a vocabulary.
    fake = ScriptedAudioClassifier(label="speech")
    res = analyze_audio(
        Path("clip.wav"),
        classifier=fake,
    ...
    assert res.segments  # ran fine; ScriptedAudioClassifier ignores candidate_labels
...
def _analysis():
    seg = lambda i, lbl: AudioSegment(  # noqa: E731
        i,
        float(i),
        float(i + 1),
...
@pytest.mark.skipif(
    not os.environ.get("ARGUS_RUN_MODEL_TESTS"),
    reason="set ARGUS_RUN_MODEL_TESTS=1 to run (downloads the default CLAP model)",
)
def test_huggingface_backend_classifies_a_tone():
    ...
    clf = HuggingFaceAudioClassifier(device="cpu")  # default is zero-shot CLAP
    assert clf.is_zero_shot
    t = np.linspace(0, 1, 16000, endpoint=False, dtype=np.float32)
    tone = np.sin(2 * np.pi * 440 * t).astype(np.float32)
    preds = clf.classify(tone, 16000, top_k=2, candidate_labels=["a tone", "speech"])
