...
def test_search_filters_prefilter_in_scan(store):
    ...
    ids = [
        h.sighting.track_id
        for h in store.search_sightings(
            _unit(0), "fake_v1", top_k=10, cameras=["camA"], since=5.0, min_quality=0.5
        )
    ]
...
def test_enroll_and_watchlist_search(store):
    ...
    store.add_enrollment(
        Enrollment(
            identity_id=iid, chip_path="/e.png", embedding_space_id="fake_v1", source="id_photo"
        ),
        _unit(0),
    )
    hits = store.search_enrollments(
        np.array([0.97, 0.24, 0, 0, 0, 0, 0, 0], np.float32), "fake_v1", top_k=3
    )
    assert len(hits) == 1
